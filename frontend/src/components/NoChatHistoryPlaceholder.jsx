...
const NoChatHistoryPlaceholder = ({ name, onSendMessage }) => {
  const handleSendSuggestedMessage = (message) => {
    if (onSendMessage) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-6">
      ...
      <h3 className="text-lg font-medium text-slate-200 mb-3">
        Start your conversation with {name || "your friend"}
      </h3>
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        <button 
          onClick={() => handleSendSuggestedMessage("👋 Hello!")}
        ...
        <button 
          onClick={() => handleSendSuggestedMessage("🤝 How are you?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors cursor-pointer"
        >
          🤝 How are you?
        </button>
        <button 
          onClick={() => handleSendSuggestedMessage("📅 Want to meet up soon?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors cursor-pointer"
        >
          📅 Meet up soon?
        </button>
